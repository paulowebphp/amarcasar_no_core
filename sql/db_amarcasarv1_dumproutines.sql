...
BEGIN
    ...
    ELSE
		...
		INSERT INTO tb_addresses (iduser, idcart, desaddress, desholderaddress, desnumber, desholdernumber, descomplement, desholdercomplement, descity, desholdercity, desstate, desholderstate, descountry, desholdercountry, deszipcode, desholderzipcode, desdistrict, desholderdistrict)
        VALUES(piduser, pidcart, pdesaddress, pdesholderaddress, pdesnumber, pdesholdernumber, pdescomplement, pdesholdercomplement, pdescity, pdesholdercity, pdesstate, pdesholderstate, pdescountry, pdesholdercountry, pdeszipcode, pdesholderzipcode, pdesdistrict, pdesholderdistrict);
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_bestfriends_update`(`pidbestfriend` INT(11), `piduser` INT(11), `pinstatus` TINYINT, `pinposition` TINYINT, `pdesbestfriend` VARCHAR(128), `pdesdescription` TEXT, `pdesphoto` VARCHAR(256), `pdesextension` VARCHAR(4))
BEGIN
...
/*!50003 SET sql_mode              = 'NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_carts_save`(`pidcart` INT, `pdessessionid` VARCHAR(64), `piduser` INT, `pincartstatus` TINYINT(4))
BEGIN
...
BEGIN
	...
	IF pidplan > 0 THEN
        ...
        SET
            ...
            insellercategory = pinsellercategory,
            idcupom = pidcupom,
            incupom = pincupom,
...
`piduser` INT(11), 
`pinclosed` TINYINT, 
`pinnameconfirm` TINYINT, 
`pinaccompaniesconfirm` TINYINT, 
...
BEGIN
	...
	IF pidrsvpconfig > 0 THEN
        ...
        SET           
			inclosed = pinclosed,
			innameconfirm = pinnameconfirm,
            inaccompaniesconfirm = pinaccompaniesconfirm,
...
/*!50003 SET sql_mode              = 'NO_AUTO_VALUE_ON_ZERO' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_weddings_update`(`pidwedding` INT(11), `piduser` INT(11), `pdesweddingdescription` TEXT, `pdesweddinglocation` VARCHAR(128), `pdesweddingphoto` VARCHAR(256), `pdtwedding` DATETIME)
BEGIN
...
-- Dump completed on 2019-05-23 11:13:09
